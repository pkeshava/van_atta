...
d = 0.5*lamda;                                              % element seperation
theta_i = pi/4;                                             % incident angle on array
i_phi = 2*pi/lamda*d*cos(theta_i);                          % relative incident phases
...
for i = 1:size(i_phi_vector,2)
    for t = 1:size(theta,2)
        AF_values(i,t) = exp(1i*(2*pi/lamda*d*cos(theta(t)) - i_phi_vector(i)));
    end
end
